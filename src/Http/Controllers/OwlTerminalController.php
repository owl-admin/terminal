...
namespace Slowlyo\OwlTerminal\Http\Controllers;
...
use Slowlyo\OwlTerminal\OwlTerminalServiceProvider;

class OwlTerminalController extends AdminController
{
    ...
    public function page()
    {
        ...
        return amisMake()->Card()->body([
            ...
            amisMake()
                ->Form()
                ->id('terminal_form')
                ->api(admin_url('dev_tool_terminal/preload'))
                ->wrapWithPanel(false)
                ->body([
                    ...
                    amisMake()->GroupControl()->direction('vertical')->body([
                        ...
                        amisMake()
                            ->Service()
                            ->schemaApi(admin_url('dev_tool_terminal/artisan_schema?command=${command}')),
                    ...
                    amisMake()->GroupControl()->direction('vertical')->body([
                        ...
                        amisMake()
                            ...
                            ->selectMode('group')
                            ->required(true)
                            ->source(admin_url('dev_tool_terminal/composer_options'))
                            ->searchable(true)
                            ->visibleOn('${action_type != "require"}'),
                    ...
                    amisMake()
                        ...
                        ->body([
                            ...
                            amisMake()->Log()->className('mt-3')->height(600)->source([
                                'url'        => url(admin_url('dev_tool_terminal/exec?uuid=${uuid}', true)),
                                'expression' => '${uuid}',
                                'method'     => 'post',
    ...
    public function preload(Request $request)
    {
        ...
        $settingKey = $type == 'artisan' ? 'php' : 'composer';
        $alias      = OwlTerminalServiceProvider::setting($settingKey . '_alias', $settingKey);
